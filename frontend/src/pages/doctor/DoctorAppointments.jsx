  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
            ...
            return (
              <div key={apt._id} className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col group">
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-100 flex flex-wrap gap-2 justify-end items-center">
                  ...
                  <button 
                    onClick={() => { setAptToRemove(apt); setIsDeleteModalOpen(true); }}
                  ...
                  >
                    <Trash2 className="w-4 h-4" /> {apt.status === 'completed' ? 'Remove' : 'Cancel'}
                  </button>
                </div>
      ...
      <DeleteConfirmationModal 
        ...
        onClose={() => { setIsDeleteModalOpen(false); setAptToRemove(null); }}
        onConfirm={handleRemoveAppointment}
        recordName={`Appointment with ${aptToRemove?.patient?.name || 'patient'}`}
