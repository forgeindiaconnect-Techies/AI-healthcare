...
import { Calendar, Clock, Video, CheckCircle, XCircle, RefreshCw, Plus, Video as VideoIcon, Building2, Ticket, FileText, UserCircle, Users, Stethoscope, MessageSquare, Trash2 } from 'lucide-react';
...
const DoctorAppointments = () => {
  ...
  const handleRemoveAppointment = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await API.patch(`/api/appointments/${aptToRemove._id}/remove`, {}, config);
      ...
      toast.error('Unable to remove appointment. Please try again.');
      setRemoveModalOpen(false);
      setAptToRemove(null);
    }
  };
            ...
            return (
              <div key={apt._id} className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col group">
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-100 flex flex-wrap gap-2 justify-end items-center">
                  ...
                  <button 
                    onClick={() => { setAptToRemove(apt); setRemoveModalOpen(true); }}
                    className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-sm font-bold flex items-center gap-2 transition-all ml-auto border border-red-100"
                    title="Remove Appointment"
                  >
                    <Trash2 className="w-4 h-4" /> {apt.status === 'completed' ? 'Remove from list' : 'Remove'}
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-lg">
            ...
            <DoctorChat 
              ...
              onClose={() => { setChatModalOpen(false); setSelectedAppointmentForChat(null); }} 
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl border border-gray-100 animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 bg-red-50/30 flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-gray-900">Remove Appointment?</h2>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button 
                  onClick={() => { setRemoveModalOpen(false); setAptToRemove(null); }}
                  className="px-5 py-2.5 rounded-xl font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-all border border-transparent"
                >
                  Cancel
                </button>
                <button 
                  onClick={handleRemoveAppointment}
                  className="px-5 py-2.5 rounded-xl font-bold text-white bg-red-600 hover:bg-red-700 shadow-md shadow-red-200 transition-all flex items-center gap-2"
                >
