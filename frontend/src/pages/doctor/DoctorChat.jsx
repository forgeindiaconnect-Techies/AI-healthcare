...
import { Send, Paperclip, Check, CheckCheck, Clock, FileText, Download, UserCircle } from 'lucide-react';
import toast from 'react-hot-toast';

const DoctorChat = ({ patientId, appointmentId, patientName, patientAvatar }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (socket) {
      const handleNewMessage = (msg) => {
        if (msg.sender._id === patientId || msg.sender === patientId) {
          setMessages(prev => [...prev, msg]);
          // Mark as read 
          socket.emit('read_receipt', { senderId: patientId, receiverId: user.id });
        }
      };
  ...
  const fetchChatHistory = async () => {
    try {
      const { data } = await API.get(`/api/chat/${patientId}${appointmentId ? `?appointmentId=${appointmentId}` : ''}`, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      setMessages(data.data || []);
      
      // Mark as read
      await API.put(`/api/chat/${patientId}/read`, {}, { headers: { Authorization: `Bearer ${user.token}` } });
      if (socket) socket.emit('read_receipt', { senderId: patientId, receiverId: user.id });
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="flex flex-col h-[500px] bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-4 bg-indigo-50/50 border-b border-indigo-100 flex items-center gap-3">
        {patientAvatar ? (
          <img src={patientAvatar} alt={patientName} className="w-10 h-10 rounded-full border border-white shadow-sm" />
        ) : (
          <UserCircle className="w-10 h-10 text-indigo-300" />
        )}
        <div>
          <h3 className="font-bold text-gray-900">{patientName || 'Patient'}</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/30">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="font-medium">No messages yet with {patientName}</p>
          </div>
        ) : (
          messages.map((msg, i) => {
