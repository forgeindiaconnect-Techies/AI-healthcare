...
export const Input = ({ label, type = "text", value, onChange, placeholder, icon, style = {}, name, autoComplete }) => {
  ...
  return (
    <div style={{ marginBottom: 16, ...style }}>
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
        ...
        <input type={currentType} name={name} autoComplete={autoComplete} value={value} onChange={onChange} placeholder={placeholder} style={{ width: "100%", boxSizing: "border-box", padding: icon ? "10px 12px 10px 40px" : "10px 12px", paddingRight: isPassword ? "40px" : "12px", borderRadius: 8, border: `1px solid ${colors.border}`, fontSize: 14, color: colors.text, background: colors.surface, outline: "none" }} />
        {isPassword && (
          <button 
            type="button" 
